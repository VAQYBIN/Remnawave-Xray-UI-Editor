...
describe('форма секции Mihomo', () => {
  ...
  it('значение из якоря доступно только для чтения и объясняет, почему', () => {
    renderForm()
    const field = screen.getByLabelText('interval')
    expect(field).toHaveAttribute('readonly')
    expect(screen.getByText(/приходит через/)).toBeInTheDocument()
  })
  ...
  if (origin === 'merged') {
    return (
      <TextField
        controlId={id}
        label={field.key}
        hint={`${field.doc} Значение приходит через слияние «<<:» — правится в тексте, у объявления якоря.`}
        value={String(value ?? '')}
        readOnly
        onChange={() => {}}
      />
