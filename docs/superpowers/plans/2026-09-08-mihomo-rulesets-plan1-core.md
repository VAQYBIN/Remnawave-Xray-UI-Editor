...
3. `maxOutputLength: maxPlainBytes` → убрать опцию — падает «zstd-бомба».
   Проверено на практике: тест ловит эту мутацию только потому, что бомба несёт
   ЗАКОННЫЙ заголовок. С мусором внутри снятый лимит давал бы отказ по подписи —
   того же класса, и тест оставался бы зелёным при сломанной защите.
...
`type`, `url`, `format`, `proxy`, `payload` и **`intervalSec`** и заполнить их
тем же `str`, каким уже читается `behavior`. Для `payload` подойдёт уже готовый
`strings`; второго способа читать список в этом модуле заводить не надо.

`intervalSec` — единственное числовое поле, и `str` его не отдаст: он проверяет,
что значение строковое. Понадобится маленький `num` рядом со `str`. (В первой
редакции плана этого поля в перечислении не было, хотя код шага 4 и тест его
требуют — пропуск нашёл исполнитель.)
