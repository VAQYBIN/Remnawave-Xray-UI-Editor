...
Тест про трассировку зелёный только после задачи 9 (пока `traceSingbox` принимает строку) — номер задачи здесь исправлен ревью: трассировку переводит на `TraceTarget` задача 9, а не 8. Если задачи 4 и 9 исполняются ПАРАЛЛЕЛЬНО, тест пишется сразу против новой сигнатуры и ничем не помечается. Если задача 4 идёт раньше — тест помечается `it.skip` с комментарием «ждёт задачи 9: цель трассировки станет TraceTarget», и задача 9 обязана его включить обратно. Пропуск без причины в коде запрещён — причина пишется рядом.
...
### Task 16 (заведена ревью по ходу исполнения): вход к формам набора правил и DNS-сервера

**Дефект плана, который она закрывает.** Спека требует в инспекторе шесть форм — правило
маршрута, группа, выход-сервер, вход, набор правил и DNS-сервер. Задача 7 пишет все шесть,
задача 8 разводит по узлам графа только первые четыре, и это не её недосмотр: узлов у
`route.rule_set` и `dns.servers` на холсте нет и быть не должно («`rule_set` — свойство
правила, а не узел», «DNS в граф не идёт»). Плана входа для двух оставшихся форм не назвал
ни один шаг — они оставались мёртвым кодом, а требование спеки невыполненным.

**Решение.** Вход — две кнопки дока рядом с «+ Правило», открывающие инспектор на
ПСЕВДОУЗЛЕ: `doc:rule-sets` (список `route.rule_set`) и `doc:dns-servers` (список
`dns.servers`). Псевдоузел — id, которого в графе нет; так можно, потому что `GraphCanvas`
получает `selectedId` строкой и лишь подсвечивает совпавший узел, а инспектор рисует
страница по тому же `draft.selectedNode`. Общий слой при этом не знает о псевдоузлах
ничего — в том и приём.

**Файлы:** изменить `features/topology/SingboxTopology.tsx` (кнопки дока),
`features/topology/SingboxInspector.tsx` (две ветки), `test/singbox-inspector.test.tsx`,
`test/singbox-topology.test.tsx`.

Правка записи идёт по ИНДЕКСУ, а не по тегу: у набора тег может отсутствовать или
дублироваться, и поиск по нему потерял бы правку. Пустой список объясняется утверждением о
разборе («в документе нет ни одного набора правил»), а не о файле — той же формулой, что и
пустой холст.
