...
## Долги, оставленные планом 1

Записано по итогам финального ревью ветки `feat/mihomo-core`. Ни один пункт не блокирует
работоспособность ядра; все они — обязательный вход плана 2 либо мелочи, которые дешевле закрыть
там, где до них дойдут руки.

### Обязательный вход плана 2

- **Коммутация не сообщает причину отказа.** `connectMihomo`/`disconnectMihomo` возвращают пустой
  список правок в пяти разных ситуациях: список записан в одну строку, список пришёл через
  слияние, ключа `proxies` нет, в имени перевод строки, имя уже в списке. Для форм такой механизм
  есть (`fieldOrigin` прямо говорит «значение из якоря»), для кабеля — нет: он просто отскакивает
  без объяснений. Задача UI обязана вернуть причину (`{ edits, reason }`) и показать её, иначе
  придётся разбирать причины повторно у себя. Решение отложено сюда сознательно: это выбор формы
  интерфейса, а его лучше делать, зная нужды диалогов.
- **Раскладки по вертикали нет.** `buildMihomoGraph` ставит всем узлам, кроме правил, `y: 0` —
  в колонке они лягут друг на друга. У графа Xray раскладка живёт внутри `buildGraph`; здесь её
  предстоит дописать, иначе первый же рендер выглядит как поломка графа.
- **Тип правила не прокинут в проверку допустимости соединения.** `isValidMihomoConnection`
  принимает тип правила, но снаружи его никто не передаёт — защита от перетаскивания кабеля с
  узла `SUB-RULE` сейчас держится только на самой операции. При подключении графа к React Flow
  тип нужно прокинуть, иначе кабель будет тянуться вхолостую.

### Мелочи

- `connectMihomo`/`disconnectMihomo` адресуют группу по имени; при дубликате имён работают с
  первой, тогда как `renameGroup` в такой ситуации отказывает. Расхождение терпит, пока
  коммутация не подключена к интерфейсу.
- `walkScalars` принимает два соседних булевых позиционных параметра (`insideFlow`, `dnsOnly`).
  Все вызовы верны, но перестановка не даст ни ошибки типов, ни красного теста, а на этой функции
  держится корректность переименования. Заменить на объект-опции.
- В тесте подстановки фиктивных серверов проверка отсутствия ключей `remnawave` подстрочная —
  упадёт ложно, если в фикстуре появится ссылка вида `github.com/remnawave/...`.
- `hashTemplate` для `XRAY_BASE64` считает хэш от `templateJson`, хотя комментарий в `CLAUDE.md`
  относит содержимое этого типа к `encodedTemplateYaml`. Вреда нет (`PATCH` на него отвечает
  `400`), но при появлении задачи на `XRAY_BASE64` это разойдётся первым.
- Группа, названная именем встроенной цели (`DIRECT`), даёт узел, в который никогда не ведёт
  ребро; поиск дубликата ребра линейный. Замер на большом шаблоне: весь граф строится за 1.8 мс.
- Нет автотеста на приоритет внутри списка якорей (`<<: [*a, *b]`); поведение верное, проверено
  вручную.
- Неразрешённый алиас в правиле (`*r1` без объявленного якоря) даёт предупреждение, а не ошибку.
  Это следствие требуемого поведения — редактор честно говорит, что проверить не может, — и ядро
  всё равно поймает при проверке.

### Не из этой ветки — закрыто по ходу

- `frontend/test/intellisense.test.ts`, тест про большой документ, падал примерно раз из трёх
  полных прогонов и тем обесценивал зелёный прогон как доказательство. Причина не в продукте, а в
  зависимости теста от стенных часов: бюджет разбора в 100 мс под полной нагрузкой не
  выдерживается, воркер вытесняется ОС, и резолвер честно откатывается на устаревший снимок
  дерева — в продукте это «подсказка не появилась на этом нажатии», а не поломка.
  Закрыто повтором попытки (`retry: 2`) с объяснением в коде: настоящий регресс — если резолвер
  снова начнёт читать снимок вместо `ensureSyntaxTree` — падает детерминированно и повтором не
  спасается (проверено саботажем: три попытки из трёх красные), а вытеснение случайно и проходит
  на повторе.
