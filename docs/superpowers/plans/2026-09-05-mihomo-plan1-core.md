...
- `frontend/test/intellisense.test.ts`, тест про большой документ, неустойчив под полной
  нагрузкой: одно падение из трёх прогонов, `result` оказывается `undefined`. Причина не в
  продукте, а в тесте: он зависит от настенного времени, а бюджет разбора в 100 мс под нагрузкой
  не выдерживается, и резолвер честно откатывается на устаревший снимок дерева. В продукте это
  деградация до «подсказка не появилась на этом нажатии». Чинить прогревом дерева в arrange-части
  теста либо параметризацией бюджета — отдельной работой, к Mihomo отношения не имеет.
