...
**Все закрыты 2026-09-05**, до начала работ по Mihomo. Список оставлен как
запись о том, что именно чинилось: ревью пачки нашло сверх него ещё шесть
дефектов, из них один блокирующий (`addInjectGroup` занимала тег соседней
группы) и один — не в тесте, а в коде подсказок (`syntaxTree(state)` отдаёт
снимок, который `ensureSyntaxTree` не обновляет).

Собраны были по итогам финального ревью ветки и живой проверки. Ни один не
ломал работающий код — это места, где решение было отложено сознательно.
