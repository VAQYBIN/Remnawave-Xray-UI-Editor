...
**8. `MihomoNameField` коммитит новое имя по потере фокуса и по Enter, а не по каждому нажатию (задача 11, пересмотрено финальным ревью).** Код брифа плана писал `onRename` на каждый символ, как строковые поля `SchemaForm`; исполнение сначала оставило это (цена ошибки казалась — один снимок истории на букву). Финальное ревью измерило настоящую цену: каждая ссылка на имя — отдельная операция режима модели с перепечаткой ВСЕГО документа, 0,4–0,9 с на клавишу на шаблоне из нескольких сотен строк; и хуже — при отказе на итоговом имени (`taken`) в документе оставалось последнее успешное промежуточное имя (`US` занято → запись названа `U`). Поле держит локальный буфер, коммитит по blur/Enter, Escape возвращает буфер к значению документа и гасит всплытие (глобальный Escape закрывает инспектор). Это возврат к поведению `TextRow … onCommit` прежней `MihomoFieldsForm`, а не новое ограничение.
...
**12. `insert` по существующему ключу с пустым значением (`proxies:` без списка) заводит список, а не отказывает (задача 6, финальное ревью).** Панельный каркас `starterMihomo.ts` кладёт `proxies: # LEAVE THIS LINE!` и в корень, и в группу; для библиотеки `yaml` такой ключ существует (`hasIn` → true) со значением `Scalar(null)`, и первая версия писателя срабатывала на защите «по этому пути не список — вставка заменила бы значение» там, где заменять нечего. На каркасе панели отказывали «+ Добавить → Сервер», кабель «группа → сервер» и рецепт WARP — ровно сценарий «с нуля» из цели спеки. Пустой скаляр считается отсутствием значения; e2e «с нуля» теперь стартует с панельного каркаса, а не с пустого документа.

**13. Переименование пересобирает ключ политики `rule-set:<a>,<b>` из разобранных частей, а не через `String.replace` (задача 7, финальное ревью).** `replace(from, to)` меняет ПЕРВОЕ вхождение в сегменте, включая литерал префикса `rule-set:` — переименование набора `ru` → `ads` превращало ключ `nameserver-policy` в `adsle-set:ru` и рапортовало успех. Каждый сегмент теперь сравнивается целиком (`t === from` → `to`; `t === 'rule-set:' + from` → `'rule-set:' + to`); тот же принцип у записей `filter` и у логических payload.

**14. `maxAliasCount: -1` в снимке `md.json` сохранён, но комментарий больше не называет вход доверенным (задача 6, финальное ревью).** Через тот же разбор идёт и YAML из каталога импорта, и `toJS` считается на каждое нажатие; конечный порог отказал бы живым шаблонам с десятками алиасов, поэтому принятый риск — «алиасная бомба» из собственного индекса каталога — назван в комментарии, а не спрятан за словом «доверенный».

**15. У примитивов рецептов `ensure*` есть статус `'refused'`, и список изменений не говорит «уже есть» о записи, которую писатель отказался писать (задача 18, финальное ревью).** Первая версия сводила отказ к `exists`; заметка об отказе стояла ниже, а строка списка утверждала обратное. `RecipeChange.status` расширен аддитивно (`shared/recipes/types.ts`), Xray и sing-box его не производят.
