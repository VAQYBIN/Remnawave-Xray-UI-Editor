 ...
 * «billion laughs» — без неё `toJS` бросает на документе с несколькими
 * использованиями одного якоря раньше третьего дубля, а живые шаблоны Mihomo
 * (`x-anchors`, `<<:`-слияния) как раз так и устроены. Находка ревью: вход
 * НЕ ограничен собственным файлом пользователя — диалог импорта каталога
 * (`ImportTemplateDialog`) кладёт в этот же разбор чужой YAML по ссылке из
 * индекса готовых шаблонов (`backend/src/catalog/service.ts`), и `snapshot`
 * зовётся на каждое нажатие клавиши в текстовой вкладке. Принятый риск — не
 * «доверенный ввод», а алиас-бомба конкретно из ссылок каталога панели
 * (проверены вручную, статический список), а не из произвольного чужого
 * YAML вообще. Любое исключение (в том числе неразрешимый алиас) — `{}`, а
 * не падение формы.
 */
