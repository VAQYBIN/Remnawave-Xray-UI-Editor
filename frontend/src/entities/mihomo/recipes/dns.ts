...
import { ensureListEntry, ensureScalar } from './apply'
...
export function planDns(md: MihomoDoc, p: DnsParams): RecipePlan<MihomoDoc> {
  ...
  changes.push({ status: enable.status, text: enable.status === 'add' ? 'dns.enable = true' : 'dns.enable уже задан' })

  if (p.fakeIp) {
    const mode = ensureScalar(next, ['dns', 'enhanced-mode'], 'fake-ip')
    next = mode.md
    notes.push(...mode.notes)
    changes.push({ status: mode.status, text: mode.status === 'add' ? 'dns.enhanced-mode = fake-ip' : 'dns.enhanced-mode уже задан' })

    const range = ensureScalar(next, ['dns', 'fake-ip-range'], '198.18.0.1/16')
    next = range.md
    notes.push(...range.notes)
    changes.push({ status: range.status, text: range.status === 'add' ? 'dns.fake-ip-range = 198.18.0.1/16' : 'dns.fake-ip-range уже задан' })
  }

  const local = ensureListEntry(next, ['dns', 'default-nameserver'], p.local, 'end')
  next = local.md
  notes.push(...local.notes)
  changes.push({ status: local.status, text: local.status === 'add' ? `default-nameserver: ${p.local}` : `${p.local} уже в default-nameserver` })

  const remote = ensureListEntry(next, ['dns', 'nameserver'], p.remote, 'end')
  next = remote.md
  notes.push(...remote.notes)
  changes.push({ status: remote.status, text: remote.status === 'add' ? `nameserver: ${p.remote}` : `${p.remote} уже в nameserver` })
  ...
  for (const filter of filters) {
    ...
    changes.push({ status: res.status, text: res.status === 'add' ? `fake-ip-filter: ${filter}` : `${filter} уже в fake-ip-filter` })
  }

  const store = ensureScalar(next, ['profile', 'store-fake-ip'], true)
  next = store.md
  notes.push(...store.notes)
  changes.push({ status: store.status, text: store.status === 'add' ? 'profile.store-fake-ip = true' : 'profile.store-fake-ip уже задан' })
