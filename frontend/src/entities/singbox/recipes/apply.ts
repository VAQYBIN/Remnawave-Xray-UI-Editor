...
/**
 * Позиция сразу за ведущей серией СЛУЖЕБНЫХ правил (sniff, resolve,
 * route-options, hijack-dns). Набор нетерминальных действий берём из
 * `entities/singbox/rules.ts` (`NON_TERMINAL_ACTIONS`), а не переписываем
 * список руками: это ОДНА истина о нетерминальных действиях в проекте —
 * трассировка (`trace.ts`) и граф читают её же, и вторая копия списка
 * разошлась бы с ними на первом же новом действии ядра. `hijack-dns` в этот
 * набор НЕ входит — он, как и в `trace.ts`/графе, ЗАВЕРШАЕТ подбор, а не
 * пропускает его дальше, — но здесь он всё равно причисляется к ведущей
 * серии: рецепт `dns` кладёт его первым правилом как перехват DNS-запросов
 * до всей остальной маршрутизации, и следующий рецепт обязан вставлять свои
 * правила ПОСЛЕ него, а не перед — иначе он перехватил бы DNS раньше, чем
 * успеет сработать вставленное правило.
 */
