...
export const SINGBOX_SECTIONS: Record<SingboxSectionName, SingboxField[]> = {
  ...
  outbound: [
    { key: 'type', doc: 'Тип выхода: протокол сервера, группа выбора или прямой выход.', type: 'string', enum: [
      ...
      // `block` и `dns` здесь БЫЛИ и убраны намеренно: ядро объявило их
      // устаревшими в 1.11 и удалило в 1.13, а панель нацелена на 1.13.x —
      // предложенный тип собрал бы шаблон, который ядро не примет. Замена у
      // обоих — действие правила: reject вместо block, sniff + hijack-dns
      // вместо dns. В уже написанных документах такой тип встречается, и там
      // он не пропадает: форма пропускает текущее значение сквозь список и
      // объясняет замену, а `validate.ts` пишет о нём предупреждение.
    ] },
