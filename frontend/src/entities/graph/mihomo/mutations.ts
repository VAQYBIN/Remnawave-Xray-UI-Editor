...
const REFUSAL_TEXT: Record<MihomoRefusal, string> = {
  ...
  // подробностями отправляет в форму группы, где видны все.
  'panel-hosts-edge': 'Эту связь создаёт панель, а не документ: группа получает хосты от панели, и если панель их подставит, они попадут в группу сами. Записи в `proxies` под это ребро нет — разрывать нечего. Чем именно у этой группы задано получение хостов, видно в её форме.',
