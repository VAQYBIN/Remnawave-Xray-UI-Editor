...
const REFUSAL_TEXT: Record<MihomoRefusal, string> = {
  ...
  // Ни одно основание здесь НЕ названо намеренно. Решает `groupGetsHosts`
  // (`entities/mihomo/inject.ts`) — единственный источник правды, и оснований у
  // него не пара и не «четыре»: считать их в комментарии значит завести второй
  // список, который разойдётся с первым (уже расходился). Назвать часть — для
  ...
  // истинным при ЛЮБОМ решении `groupGetsHosts`, поэтому он говорит про факт, а
  // за подробностями отправляет туда, где основания видно.
  //
  // Мест ДВА, и назвать их обязано оба. Всё, что `groupGetsHosts` читает
