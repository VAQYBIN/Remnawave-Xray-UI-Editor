...
const REFUSAL_TEXT: Record<MihomoRefusal, string> = {
  ...
  'panel-hosts-edge': 'Эту связь создаёт панель, а не документ: узел подстановки нарисован по маркеру `# LEAVE THIS LINE!` (или по ключу `include-all` группы), и если панель подставит сюда хосты, они попадут в группу сами. Записи в `proxies` под неё нет, разрывать нечего. Уберите маркер или `include-all` в форме группы.',
