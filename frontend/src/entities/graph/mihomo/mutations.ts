...
const REFUSAL_TEXT: Record<MihomoRefusal, string> = {
  ...
  // Оснований, по которым появляется узел подстановки, четыре (`groupGetsHosts`:
  // маркер, `include-all`, `select-random-proxy`, `shuffle-proxies-order`), и ни
  // одно здесь НЕ названо намеренно. Перечислить пару частых значило бы для
  // остальных групп назвать основание, которого у них нет: писатель пошёл бы
  // искать в документе маркер, а маркера там не будет. Текст обязан оставаться
  // истинным при любом из четырёх, поэтому он говорит про факт, а за
  // подробностями отправляет туда, где эти основания видно.
  //
  // Мест ДВА, и назвать их обязано оба. Три основания из четырёх — ключи
  // (`include-all`, `remnawave.select-random-proxy`,
  // `remnawave.shuffle-proxies-order`), их показывает форма группы. Четвёртое —
  // КОММЕНТАРИЙ `# LEAVE THIS LINE!` внутри списка `proxies`: полем формы он не
  // является и в ней не появится вовсе. Отправив только в форму, мы завели бы
  // писателя с маркером в тупик — он увидел бы все флаги выключенными.
