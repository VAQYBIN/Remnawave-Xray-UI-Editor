...
export interface ValidationIssue {
  ...
  level: 'error' | 'warning'
  /**
   * Точное место в тексте, когда путь назвать нечем. Такова синтаксическая
   * ошибка разбора: сказать «где» она может, а «что за ключ» — нет, потому
   * что документ на этом месте и не разобрался. Без этого поля клик по ней
   * в списке проблем не вёл никуда: список навигирует по пути, а путь пуст.
   * У проверок с путём поля нет — место им ищет резолвер по документу.
   */
