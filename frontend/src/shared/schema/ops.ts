 ...
 * `depth` — сколько шагов пройти, а `path` передаётся ПОЛНОСТЬЮ, а не
 * обрезанным до `depth`: так следующий шаг (`nextStep`) виден и на последнем
 * пройденном звене, и `set` с числовым последним сегментом пути заводит
 * список, а не объект.
 */
