...
export interface EditorShellProps {
   ...
   * таким, каким он был.
   *
   * `singbox-json` — не третий ФОРМАТ, а диалект JSON: подпись вкладки и тип
   * файла у него те же, различается только разбор ЗАГРУЖЕННОГО файла (конфиг
   * Xray, положенный в редактор sing-box, обязан быть отвергнут, а не молча
   * стать черновиком). Отдельным пропом «диалект» это была бы вторая истина о
   * документе рядом с docFormat, и разъехаться они могли бы молча.
   */
