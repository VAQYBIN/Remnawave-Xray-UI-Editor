...
// заново целиком и уходит писателю одной операцией `set` по пути правила —
// тем же, что адресует и основной список `rules`, и любой подсписок под
// `sub-rules.<имя>`: форма о месте, где живёт список, ничего не знает.
...
function RuleFields({
  ...
  const commit = (next: MihomoRule, field: RuleField) => {
    ...
    setRejected(null)
    writer.apply([{ op: 'set', path, value: formatRule(next) }])
  }

  // Кандидаты цели приходят от вызывающего готовым списком (`DocRefs`): у
  // SUB-RULE это имена подсписков, иначе — теги групп, серверов и встроенных
  // целей, уже собранные схемой в один список `proxy-target`.
  const known = isSubRule ? refs['sub-rule'] ?? [] : refs['proxy-target'] ?? []
...
export function MihomoRuleForm({
  ...
  if (rule === null) {
    ...
    return (
      <p className="muted">
        Строку правила разобрать не удалось — правьте её на вкладке YAML: «{raw}».
      </p>
    )
  }
  // key — ПУТЬ правила, и сбрасывает он локальные состояния (`rejected`,
  // `customTarget`) ровно при переходе на другое правило (или на то же место в
  // другом списке), а не на любую правку строки. Ключом по СОДЕРЖИМОМУ правила
  // это не лечится: форма пишет на каждое нажатие клавиши, строка меняется
  // вместе с ним, и поле пересоздавалось бы под кареткой.
  return <RuleFields key={path.join('.')} path={path} rule={rule} writer={writer} refs={refs} />
}
