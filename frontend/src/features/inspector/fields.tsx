 ...
 * бы в уже очищенное поле.
 *
 * Буфер ресинхронизируется с `value` В РЕНДЕРЕ (паттерн производного
 * состояния: `useState` + сравнение с прошлым значением прямо в теле
 * компонента), а не только при монтировании. Причина — карточки `ListEditor`
 * ключуются индексом позиции (`key={i}`), а не тождеством элемента: при
 * перестановке (`reorder`) и в схемной форме, где то же место в дереве формы
 * занимает другой узел документа, `NumberField` НЕ размонтируется — меняется
 * только его `value`. Без ресинхронизации поле продолжало бы показывать число
 * прежнего хозяина позиции, пока пользователь сам не начнёт печатать.
 *
 * `lastValue` в обработчике `onChange` НЕ обновляется — только внутри самой
 * ветки ресинхронизации, ПОСЛЕ того как `value` фактически подтвердил
 * коммит. Обновлять его сразу в обработчике опасно: `setText` и такое
 * обновление легли бы в один батч и отрендерились ДО того, как проп `value`
 * успел бы дойти от родителя, — ветка увидела бы «внешнее» расхождение
 * между уже обновлённым `lastValue` и всё ещё старым `value` и откатила бы
 * только что набранный текст его же собственным незавершившимся эхом. Раз
 * `lastValue` продолжает указывать на значение, с которым буфер СИНХРОНЕН, а
 * не на то, что он только что отправил наружу, — недописанная часть числа
 * («-», «-2.») сравнивается с тем же `value`, что и до неё, ветка не
 * срабатывает, и текст переживает как отказ `onChange` на частичный ввод,
 * так и последующий приход настоящего эха (тогда `value` и `text` совпадают
 * дословно, и повторная запись `text` — no-op).
 */
