...
describe('правки полей секций', () => {
  ...
  // Ревью раунд 2, находка 1: ключ свой, но его ЗНАЧЕНИЕ — ссылка. Диапазон
  // значения здесь — токен `*n`, и замена по нему стирает саму ссылку, подменяя
  // её литералом: авторское объявление якоря молча теряет потребителя, а
  // пользователь об этом даже не узнаёт. Читать при этом можно и нужно.
  it('скаляр через ссылку на якорь не пишется, но читается', () => {
    const text = ['x-anchors:', '  n: &n 300', 'proxy-groups:', '  - name: A', '    interval: *n', ''].join('\n')
    const md = parseMihomo(text)
    ...
    // Соседний обычный ключ той же секции по-прежнему пишется: отказ поточечный
    expect(setFieldAt(md, ['proxy-groups', 0], 'type', 'select')).not.toEqual([])
  })

  it('setListAt отказывает, когда значение ключа — алиас на список (bundle.yaml, находка 2)', () => {
    // dns.default-nameserver в bundle.yaml задан `*dns_ru`: ключ на месте, но
    // его значение — ссылка, и ветка «пустого списка» приняла бы диапазон
    // алиаса за отсутствующий список и дописала бы новый блок РЯДОМ с ним,
    // оставив сам алиас на строке ключа — синтаксически битый документ.
    // Воспроизведено на боевой фикстуре, а не на синтетике.
    //
    // Ожидание `origin` здесь правлено в ревью раунд 2: строка кодировала
    // ДЕФЕКТ. Ключ со значением-ссылкой возвращал `'own'`, и на этом основании
    // форма считала его обычным правимым полем — `setFieldAt` заменял диапазон
    // токена `*dns_ru` литералом и молча уничтожал авторскую ссылку. Теперь
    // такой ключ — `'alias'`: писать нельзя, читать можно. Отказ `setListAt`,
    // ради которого тест написан, не изменился.
    const text = mihomoFixture('bundle')
    const md = parseMihomo(text)
    expect(originAt(md, ['dns'], 'default-nameserver')).toBe('alias')
