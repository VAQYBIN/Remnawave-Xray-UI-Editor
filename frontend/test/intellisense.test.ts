 ...
 *
 * `ensureSyntaxTree` отмеряет бюджет по `Date.now()` — внутри это
 * `endTime = Date.now() + until`, а цикл разбора выходит либо по завершении,
 * либо когда часы это время перешагнут. Документы здесь крошечные и стоят
 * микросекунды процессорного времени, но под нагрузкой воркер вытесняется ОС
 * на секунды, бюджет истекает, дерево возвращается неполным — и тест падал не
 * на дефекте, а на занятости машины. Запас бюджета этого не лечил: бюджет
 * измеряется не в той валюте, в какой тратится.
 *
 * С замороженными часами бюджет не истекает никогда, и цикл выходит только по
 * завершении разбора — то есть детерминированно. Замораживаем ТОЛЬКО разбор:
 * снимок дерева строится при создании EditorState тем же способом и по тем же
 * часам, и заморозь мы его — снимок вышел бы полным, а тест «резолвер обязан
 * дотягивать дерево сам» потерял бы предмет.
 */
...
function build(src: string) {
  ...
  // Проверки «уложился ли в бюджет» здесь больше нет, и это не упущение: с
  // замороженными часами не уложиться нельзя. Число передаём любое ненулевое —
  // истечь ему не по чему
  withFrozenClock(() => ensureSyntaxTree(state, doc.length, 100))
  return { state, pos }
}
...
describe('большой документ', () => {
  it('подсказки работают в хвосте, куда начальный разбор не дошёл', () => {
    ...
    // Состояние создаётся при ЖИВЫХ часах: начальный тайм-слайс на таком
    // документе до хвоста не доходит, и снимок остаётся коротким. Именно это и
    // делает тест различающим — с полным снимком отличить «резолвер дотянул
    // дерево» от «дерево и так было готово» стало бы нечем
    const state = EditorState.create({ doc, extensions: [json()] })
    const ctx = new CompletionContext(state, doc.length, true)
    // А сам разбор в резолвере — при замороженных: под нагрузкой его
    // 100-миллисекундный бюджет истекал, резолвер честно откатывался на
    // устаревший снимок, и тест краснел на занятости машины. Прежде это
    // прикрывалось повтором, и он не помогал: падали все три попытки
    const result = withFrozenClock(() => makeCompletionSource('config')(ctx))
