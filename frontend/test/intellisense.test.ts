...
describe('большой документ', () => {
  // Повтор здесь не маскирует дефект, а отделяет его от помехи. Бюджет разбора в
  // резолвере (100 мс) отмеряется по СТЕННЫМ часам: под полной нагрузкой воркер
  // вытесняется ОС, бюджет истекает, и резолвер честно откатывается на устаревший
  // снимок — в продукте это «подсказка не появилась на этом нажатии», а не поломка.
  // Без повтора тест падал примерно раз из трёх полных прогонов и тем обесценивал
  // зелёный прогон как доказательство.
  //
  // Импликация тут ОДНОСТОРОННЯЯ, и читать её наоборот нельзя: настоящий регресс
  // (резолвер снова читает снимок вместо ensureSyntaxTree) валит все три попытки —
  // но «упали все три» регресса НЕ доказывает. Измерено: под нагрузкой этой ветки
  // заведомо исправный тест упал все три раза. Красный здесь — повод посмотреть на
  // резолвер и на загрузку машины, а не вывод о коде.
  it('подсказки работают в хвосте, куда начальный разбор не дошёл', { retry: 2 }, () => {
