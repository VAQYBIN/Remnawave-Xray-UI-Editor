...
describe('большой документ', () => {
  ...
  // Настоящий регресс (резолвер снова читает снимок вместо ensureSyntaxTree) даёт
  // отказ ДЕТЕРМИНИРОВАННО и завалит все попытки; вытеснение — случайно и проходит
  // на повторе. Без повтора тест падал примерно раз из трёх полных прогонов и тем
  // обесценивал зелёный прогон как доказательство.
