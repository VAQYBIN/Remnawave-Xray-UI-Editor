...
// `applyMihomoOps` отказывал вставке до фикса C1; сценарий заводит сервер
// кнопкой «+ Добавить» (он вставляется в корневой голый `proxies:`), то есть
// проходит через этот путь взаправду. Условие готовности страницы — кнопка
// «+ Добавить» в доке, а не пустой граф: у каркаса уже есть узел группы.
test.describe('с нуля', () => {
  ...
  test('шаблон собирается кнопками и формами, вкладка YAML не открывается', async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'Сохранить в панель' }).click()
    // MATCH в конце `rules` документ не теряет — его несёт сам каркас
    // (`MIHOMO_STARTER_YAML`), а «+ Правило» вставляет новую запись ПЕРЕД
    // ним (`nextRulePlacement`). Диалог сохранения не блокирует запись даже
    // при оставшихся предупреждениях, а его кнопка тогда лишь меняет подпись
    // на «Сохранить всё равно» — регэксп /^Сохранить/ покрывает оба случая.
