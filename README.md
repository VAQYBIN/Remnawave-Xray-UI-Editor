...
- **Импорт из каталога.** Кнопка «Импорт» в обоих редакторах шаблонов читает индекс готовых
...
- Остальные типы шаблонов (Clash, Stash, Sing-box и прочие) видны в списке, но правятся в панели:
...
не трогается, состояние читается из кэша сервера. Строка набора показывает вид (`http`/`inline`),
...
(до 1000 записей на страницу, как у просмотрщика geo-баз) — у набора доменов показаны сразу обе
...
Отчёт проверки Mihomo при обоих вердиктах — и при успехе, и при отказе — несёт оговорку: ядро
смотрело не ваш документ дословно. Настоящих
серверов в шаблоне нет (их подставляет панель), поэтому перед запуском вместо них подставляются
фиктивные прокси, их имена дописываются в группы, а секции `remnawave` вырезаются. Значит ядро
может назвать имя или позицию, которых в вашем файле нет, а успех говорит о структуре документа,
но не о работоспособности конкретных серверов.
