...
![tests](https://img.shields.io/badge/tests-2275_passing-brightgreen)
...
- **Шаблоны Sing-box.** Тип `SINGBOX` открывается своим редактором на том же адресе: граф из
  входов, правил, групп и выходов, формы по словарю ключей, вкладка JSON с автоподсказками и
  ...
  на карточке выхода, а не стрелкой: у него нет узла-источника — это либо `route.final`, либо
  просто первый элемент `outbounds`, и увидеть второе в тексте глазами почти нельзя.
