...
![tests](https://img.shields.io/badge/tests-2661_passing-brightgreen)
...
- **Шаблоны подписки.** Редактор правит и второй раздел панели — шаблоны подписки (тип
  `XRAY_JSON`) тем же графом и теми же формами, что и конфиг-профили: группы подстановки хостов
  (`remnawave.injectHosts`), которые панель превращает в реальные outbound'ы при выдаче подписки,
  видны узлами графа наравне со статическими выходами.
- **Шаблоны Mihomo.** Тип `MIHOMO` открывается своим редактором на том же адресе: граф из правил,
  групп, провайдеров и мест подстановки хостов, формы по словарю ключей, вкладка YAML с
  автоподсказками и наведением, трассировка «куда пойдёт трафик» и проверка ядром `mihomo`.
  Источник истины — сам ТЕКСТ YAML: правки идут точечными вставками, поэтому якоря, `<<:`-слияния,
  комментарии и маркеры `# LEAVE THIS LINE!` живого шаблона переживают редактирование без потерь.
  Значение, пришедшее через якорь или слияние, форма показывает только на чтение и объясняет
  почему — править его нужно там, где оно объявлено. Имена хостов, которые подставит панель,
  редактору неизвестны: всё, что о них сказано в интерфейсе, сказано условно, а ссылка на
  незнакомое имя — предупреждение, а не ошибка.
- **Шаблоны Sing-box.** Тип `SINGBOX` открывается своим редактором на том же адресе: граф из
  входов, правил, групп и выходов, формы по схеме ключей, вкладка JSON с автоподсказками и
  наведением, трассировка «куда пойдёт трафик» и проверка ядром `sing-box`. Полос на холсте
  четыре, а колонок больше: группы ссылаются на группы, и полоса групп разворачивается ровно на ту
  глубину, какую задал ваш документ. Список выходов группы редактор показывает только на чтение,
  пока его заполняет панель: она перезаписывает такой список целиком, и правка исчезла бы при
  первой же выдаче подписки — чтобы вести список руками, закрепите его ключом
  `remnawave.includeProxies: false` прямо в форме группы. Маршрут по умолчанию помечен подписью
  на карточке выхода или группы, а не стрелкой: у него нет узла-источника — это либо `route.final`,
  либо просто первый элемент `outbounds`, и увидеть второе в тексте глазами почти нельзя. Шаблон
  собирается целиком через интерфейс: кнопки «+ Добавить» и «+ Правило» заводят записи с рабочим
  минимумом полей, панель «Документ» открывает формы настроек без узла на графе (`log`, `dns`,
  `route`, наборы правил, DNS-серверы и DNS-правила) — вкладка JSON остаётся для просмотра,
  экзотики и аварийных случаев. Рецепты доступны и здесь — той же кнопкой «+ Рецепт», что у
  профиля: разделение трафика по наборам правил, блокировка рекламы, DNS с fake-ip, локальный
  вход, локальные сети напрямую, WARP-эндпоинт.
- **Импорт из каталога.** Кнопка «Импорт» во всех трёх редакторах шаблонов читает индекс готовых
  шаблонов [remnawave/templates](https://github.com/remnawave/templates), показывает содержимое
  до применения и кладёт выбранный шаблон **в черновик**, а не в панель: посмотреть, поправить,
  откатить `Ctrl+Z` и только потом сохранить. Скачивается ровно та ссылка, что пришла в индексе.
- Остальные типы шаблонов (Clash, Stash, Xray-base64) видны в списке, но правятся в панели:
  их содержимое редактор не разбирает.
...
В шаблоне Mihomo правила `RULE-SET` ссылаются на наборы из `rule-providers` — чужие файлы по
ссылке. Трассировщик спрашивает их у сервера: тот скачивает набор, разбирает (`.mrs`, YAML и
текст) и отвечает по конкретной цели «да», «нет» или отказом с причиной. Наборы `behavior:
classical` приезжают строками и считаются в браузере теми же правилами, что и правила документа.

Скачанное лежит в `DATA_DIR/rulesets` (тот же том, что бэкапы и geo-базы) и переживает
перезапуск. Срок годности — `interval` из документа, но не чаще одного обращения к серверу в
час. Место освобождается вытеснением по давности **обращения**, а не загрузки: у `private-ips`
в живых шаблонах `interval` месячный, и по дате загрузки вылетал бы как раз самый нужный файл.

Лимиты — по одному набору, а не на весь запрос: превышение даёт `unavailable` с причиной именно
у того набора, который её вызвал, а остальные отвечают как обычно.

| Лимит | Значение |
|---|---|
| Файл на проводе | 8 МБ |
| После распаковки | 32 МБ |
| Наборов на документ | 64 |
| Кэш на диске | 256 МБ |
| Разобранное в памяти | 64 МБ |
| Строк `classical` в ответе | 10 000 |
| Минимальный TTL | 1 час |

Ссылки набора проходят ту же защиту, что и geo (`net/guard.ts`): во внутреннюю сеть сервер не
ходит. Набор `type: file` лежит у клиента — сервер его не видит и честно говорит об этом, не
делая вида, что проверил.

> [!NOTE]
> Если у провайдера задано поле `proxy`, клиент качает набор через указанную группу, а редактор —
> напрямую со своего сервера. Содержимое по одной и той же ссылке из разных точек мира может
> отличаться; трассировка в этом случае дописывает оговорку.

Кнопка «Наборы правил» в топбаре открывает диалог со всеми наборами документа — сеть при этом
не трогается, состояние читается из кэша сервера. Строка набора показывает
`behavior`/`format`, когда он загружен и сколько занимает; «Обновить» перекачивает один набор,
«Обновить все» — сразу все спрошенные. Клик по имени открывает содержимое постранично с поиском
(по 200 записей на страницу, как у просмотрщика geo-баз) — у набора доменов показаны сразу обе
формы, которые ядро хранит на каждый домен (`example.com` и `+.example.com`), поэтому строк вдвое
больше, чем число записей в заголовке. Набор, который редактор скачать не может (`file`,
незнакомый вид, сетевая ошибка), остаётся в списке со своей причиной — спрятать его значило бы
соврать, что набора нет в документе; сохранение он не блокирует, это предупреждение, а не ошибка.
...
Шаблоны подписки Mihomo проверяются второй стадией того же образа: `MIHOMO_BIN=/usr/local/bin/mihomo`,
версия **v1.19.30** (`MIHOMO_VERSION` и `MIHOMO_SHA256_*` в `Dockerfile`, тот же приём с проверкой
хэша, что и у Xray). Как и с `xray`, без бинаря (обычный случай при локальной разработке) редактор
сообщает, что проверка недоступна, и продолжает работать: кнопка «Проверить ядром» открывает
отчёт, а в нём стоит плашка «бинарь mihomo не найден» — о самом шаблоне это не говорит ничего.

Шаблоны Sing-box — третьей стадией того же образа: `SINGBOX_BIN=/usr/local/bin/sing-box`, версия
**v1.13.21** (`SINGBOX_VERSION` и `SINGBOX_SHA256_*` в `Dockerfile`). В образ кладётся **musl**-сборка
ядра, а не «обычная»: glibc-архив в alpine не запускается вовсе. Поведение без бинаря то же, что у
`xray` и `mihomo`, — «проверка недоступна», и о самом шаблоне это не говорит ничего.

Отчёт проверки Mihomo и Sing-box при обоих вердиктах — и при успехе, и при отказе — несёт
оговорку: ядро смотрело не ваш документ дословно. Настоящих
серверов в шаблоне нет (их подставляет панель), поэтому перед запуском вместо них подставляются
фиктивные серверы, их имена дописываются в группы, а секции `remnawave` вырезаются. Значит ядро
может назвать имя или позицию, которых в вашем файле нет, а успех говорит о структуре документа,
но не о работоспособности конкретных серверов. У sing-box фиктивные выходы дописываются строго
**в конец** `outbounds`: при пустом `route.final` ядро берёт первый элемент массива, и вставка в
начало изменила бы смысл проверяемого документа.
...
- **«Версии»** — бэкапы панели и работа с файлом. У каждого бэкапа есть «Сравнить»: показывает
  ...
  файла; принимается и файл бэкапа целиком — конфиг из него достаётся сам. Чужой формат
  отклоняется с объяснением: и конфиг Xray, и шаблон Sing-box — это JSON с `inbounds` и
  `outbounds` в корне, поэтому редактор смотрит на приметы (`routing`/`policy` против
  `route`/`experimental`) — иначе один молча стал бы черновиком другого.
...
npm test -w frontend                        # frontend (vitest, jsdom): 2190 тестов
