...
- **`entities/singbox` — модель без интерфейса.** Разбор, словарь, диагностики и трассировка
  шаблона Sing-box готовы и покрыты тестами, но страницы редактора у этого типа ещё НЕТ:
  `TemplateEditorPage` по-прежнему уводит `SINGBOX` в панель, и это не недосмотр, а граница
  плана 1. Бэкенд при этом такой шаблон уже сохраняет — расхождение временное и снимается
  планом 2. Внутри: `parse.ts` (схема СКВОЗНАЯ — незнакомый ключ доезжает до текста нетронутым,
  ядро развивается быстрее словаря), `outbounds.ts` (всё знание о генераторе панели собрано
  здесь одним файлом, включая `defaultRoute`: при пустом `route.final` дефолт задаётся ПОЗИЦИЕЙ
  элемента, и глазами этого не видно), `rules.ts`, `docSchema.ts`, `validate.ts`, `trace.ts`.
  Арифметику адресов трассировка берёт общую — `entities/xray/traceMatch`; своя копия разошлась
  бы с первой на первом же IPv6.
...
- **Оболочка редактора разобрана на три слоя** и параметризована `DocumentAdapter`
  ...
  пропом подпись была бы второй истиной о формате и могла бы разъехаться молча.
...
- `docs/superpowers/specs/` — дизайн-документ проекта, `docs/superpowers/plans/` — планы реализации (все 4 выполнены). Шаблоны подписки — отдельная спека
  ...
  `features/templates`).
- Sing-box — спека `2026-09-07-singbox-templates-design.md` и два плана:
  `2026-09-08-singbox-plan1-core.md` ВЫПОЛНЕН (бэкенд целиком плюс `entities/singbox`), а
  `plan2` с интерфейсом ещё не написан. Отсюда и разрыв, о котором сказано выше: сохранять
  такой шаблон бэкенд уже умеет, открывать его редактор — пока нет.
