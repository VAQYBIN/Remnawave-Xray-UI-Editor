...
- **Оболочка редактора разобрана на три слоя** и параметризована `DocumentAdapter`
  ...
  группа этого разреза. Единственная истина о формате содержимого — проп `docFormat` у
  `EditorShell`: из него выводится и подпись текстовой вкладки (`YAML`/`JSON`), и поведение
  диалога версий; вторым пропом рядом пара молча разъехалась бы.
...
- **Словарь `entities/mihomo/docSchema.ts` питает и формы, и подсказки** — в отличие от Xray, где
  словарь (`intellisense/docSchema.ts`) и формы разведены намеренно. Причина: поле формы Mihomo —
  плоская пара «ключ → скаляр» в отображении (`MihomoFieldsForm` одна на группу, провайдера, набор
  правил, `dns`, `tun`, `sniffer`, `profile` и корень), и два описания одного и того же
...
- **Трассировка Mihomo останавливается** на правиле, которое не может проверить (`RULE-SET`,
  `PROCESS-*`, `SRC-*`, `UID`, `IN-*`, `SUB-RULE`), и прямо говорит об этом
  (`entities/mihomo/trace.ts`, панель — `features/diagnostics/MihomoTracePanel.tsx`). Молчаливый
  пропуск дал бы уверенный неверный ответ — худший из возможных.
