...
- **Шаблоны подписки.** `entities/xray/inject.ts` разбирает директиву `remnawave.injectHosts`:
  ...
  развернёт и сломает предсказание тегов. UI — `features/templates/*` (`TemplatesPage`,
...
- `expandSelector` не разворачивает префиксы селектора балансера, которые ловят инжектируемые
  теги (иначе развернуть — значит сломать предсказание группы); `blockingInjectPrefix` (разрыв
  «балансер → статический выход», пойманный тем же префиксом, что и группа) и
  `blockingGroupPrefix` (разрыв «балансер → группа» префиксом, который заодно ловит чужого
  кандидата) объясняют, когда такой разрыв неразрешим — на них строится диалог подтверждения в
  `TopologyView`.
